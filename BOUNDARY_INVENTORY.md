...
| read-only coin helpers | `getWalrusCoinBalances` and `getWalrusCoins` through wallet client helper | wallet read-only helper surface | locked retained JSON-RPC primary path | implemented through `SuiJsonRpcClient` (`getAllBalances`, `getCoins`, `getCoinMetadata`) on SDK 2.17.0; `.WORK/SUI_COIN_HELPER_MIGRATION_PREFLIGHT.md` locks Option A for the current evidence because Core API `listBalances`/`listCoins` are not lossless replacements (they drop `CoinBalance.lockedBalance` and `CoinStruct.previousTransaction`) and no product driver exists for narrowing the public helper contract | wallet helper tests |
